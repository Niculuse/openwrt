...
add_bandix() {
    local repo_url="https://github.com/timsaya/openwrt-bandix.git"
    local target_dir="$BUILD_DIR/package/openwrt-bandix"
    if [ -d "$target_dir" ]; then
        rm -rf "$target_dir"
    fi
    echo "正在添加 openwrt-bandix..."
    if ! git clone --depth 1 "$repo_url" "$target_dir"; then
        echo "错误：从 $repo_url 克隆 openwrt-bandix 仓库失败" >&2
        exit 1
    fi
    local makefile_path="$target_dir/Makefile"
    if [ -f "$makefile_path"]; then
        sed -i '/^define Package\/$(PKG_NAME)/,/^endef/ s/^endef/  DEFAULT:=y\nendef/' "$makefile_path"
    fi
    local repo_url="https://github.com/timsaya/luci-app-bandix.git"
    local target_dir="$BUILD_DIR/package/feeds/luci/luci-app-bandix"
    if [ -d "$target_dir" ]; then
        rm -rf "$target_dir"
    fi
    echo "正在添加 luci-app-bandix..."
    if ! git clone --depth 1 "$repo_url" "$target_dir"; then
        echo "错误：从 $repo_url 克隆 luci-app-bandix 仓库失败" >&2
        exit 1
    fi
    local makefile_path="$target_dir/Makefile"
    if [ -f "$makefile_path"]; then
        sed -i '/^PKG_MAINTAINER:=/a DEFAULT:=y' "$makefile_path"
    fi
}

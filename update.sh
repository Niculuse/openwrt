...
#    sed -i '/^config PACKAGE_$(PKG_NAME)_INCLUDE_SingBox/,/^config / { s/^default y if aarch64||arm||i386||x86_64$/default n/ }' $target_dir_core/Makefile
    sed -i '/^define Package\/$(PKG_NAME)\/config/i LUCI_DEPENDS+=+haproxy +geoview +v2ray-plugin +shadowsocks-rust-sslocal +shadowsocks-rust-ssserver +simple-obfs-client' $target_dir_core/Makefile
}
